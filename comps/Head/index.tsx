...
const Head = ({ title, description, keywords, canonical, image, type, children }: InitialProps) => {
	return (
      ...
      <meta name="twitter:image" content={image ? `${config.url}thumbnails/${image}` : `${config.url}${config.thumbnail}`} />
      ...
      <meta property="og:image" content={image ? `${config.url}thumbnails/${image}` : `${config.url}${config.thumbnail}`} />
      {children}
		</NextHead>
	)
}
