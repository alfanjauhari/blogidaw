...
const Head = ({ title, description, keywords, canonical, image, children }: InitialProps) => {
	return (
      ...
      <meta name="twitter:image" content={image ? `/thumbnail/${image}` : `/thumbnail/${config.thumbnail}`} />
      ...
      <meta property="og:image" content={image ? `/thumbnail/${image}` : `/thumbnail/${config.thumbnail}`} />
      {children}
		</NextHead>
	)
}
